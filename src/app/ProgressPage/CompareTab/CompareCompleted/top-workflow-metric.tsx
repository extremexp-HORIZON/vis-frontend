...
//     const completedWorkflows = workflows.data.filter(workflow => workflow.metrics && workflow.metrics[metric] !== undefined);
//     completedWorkflows.sort((a, b) => b.metrics[metric] - a.metrics[metric]);
//     const topTenWorkflows = completedWorkflows.slice(0, 10);
//     const chartData = topTenWorkflows.map(workflow => ({
//       workflowId: workflow.workflowId,
...
import { VegaLite, VisualizationSpec } from "react-vega";
import { Paper, Box, Typography, Tooltip, IconButton, FormControl, Select, MenuItem, SelectChangeEvent } from "@mui/material";
import InfoIcon from '@mui/icons-material/Info';
...
const TopWorkflowMetric = () => {
  ...
  const getTopTenWorkflowsByMetric = (metric: string) => {
    ...
    const completedWorkflows = workflows.data.filter(workflow => workflow.metrics && workflow.metrics[metric] !== undefined);
    completedWorkflows.sort((a, b) => (b.metrics[metric] || 0) - (a.metrics[metric] || 0));
    const topTenWorkflows = completedWorkflows.slice(0, 10);
    const chartData = topTenWorkflows.map(workflow => ({
      workflowId: workflow.workflowId,
      metricValue: workflow.metrics[metric],
