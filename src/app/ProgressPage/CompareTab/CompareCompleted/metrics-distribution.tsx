...
const getData = (metric: string) => {
  const lowercaseMetric = metric.toLowerCase();
  const completedWorkflows = workflows.data.filter(workflow =>
    workflow.metrics && workflow.metrics[lowercaseMetric] !== undefined &&workflow.workflowInfo.status === 'completed'
  );
  completedWorkflows.sort((a, b) => (b.metrics[lowercaseMetric] || 0) - (a.metrics[lowercaseMetric] || 0));
  console.log('eleos',completedWorkflows)
        ...
        <FormControl sx={{ m: 1, minWidth: 120, maxHeight: 120 }} size="small">
          ...
          >
            {availableMetrics.map(metric => (
              <MenuItem key={metric} value={metric}>
                <Checkbox checked={selectedMetrics.indexOf(metric) > -1} />
                <ListItemText primary={metric.charAt(0).toUpperCase() + metric.slice(1)} />
              </MenuItem>
            ))}
          </Select>
        </FormControl>
                  ...
                  marks: [
                    {
                      ...
                      encode: {
                        enter: {
                          ...
                          orient: { value: "horizontal" },
                          tooltip: { signal: "{'Metric': parent.metricName, 'WorkflowId': parent.workflowId,'Value': datum.value, 'Density': datum.density}" }
                    ...
                    },
                    {
                      ...
                      encode: {
                        enter: {
                          fill: { value: "black" },
                          width: { value: 2 },
                          tooltip: { signal: "{'Metric': parent.metricName,'WorkflowId': parent.workflowId, 'Q1': datum.q1, 'Q3': datum.q3}" }
                    ...
                    },
                    {
                      ...
                      encode: {
                        enter: {
                          ...
                          height: { value: 10 },
                          width: { value: 10 },
                          tooltip: { signal: "{'Metric': parent.metricName,'WorkflowId': parent.workflowId, 'Median': datum.median}" }
