...
export default function WorkflowTreeView() {
  ...
  const groupedByTask =
    ...
    console.log("variant:", tab?.workflowConfiguration.tasks)
    ...
    return (
        <Box sx={{ p: 2 }}>
          <SimpleTreeView defaultExpandedItems={["workflow-details", "tasks-root"]}>
            <TreeItem2
              itemId="workflow-details"
              label={<Typography sx={{ fontWeight: 600 }}>Workflow Details</Typography>}
            >
                
              <TreeItem2
                itemId="tasks-root"
                label={<Typography sx={{ fontWeight: 500 }}>Tasks</Typography>}
              >
                
        {Object.entries(groupedByTask).map(([taskName, datasets]) => {
  const paramsForTask = tab?.workflowConfiguration.params?.filter(p => p.task === taskName) || []
  const metricsForTask = tab?.workflowMetrics.data?.filter(m => m.task === taskName) || []
  const taskVariants: Record<string, string> = tab?.workflowConfiguration.tasks?.reduce((acc, task) => {
    if (task.name !== task.variant) {
  ...
  return (
    <TreeItem2
      ...
      <TreeItem2
        ...
        label={<Typography sx={{ fontWeight: 500 }}>Data Assets</Typography>}
      >
            ...
            label={
              ...
              >
                ...
                <Typography variant="body2" sx={{ ml: 1 }}>
                  {ds.dataset} {ds.role === "OUTPUT" ? "[out]" : ds.role === "INPUT" && "[in]"}
                </Typography>
              </Box>
            }
          />
      ...
      {paramsForTask.length > 0 && (
        <TreeItem2
          ...
          label={<Typography sx={{ fontWeight: 500 }}>Parameters</Typography>}
        >
      ...
      {metricsForTask.length > 0 && (
        <TreeItem2
          ...
          label={<Typography sx={{ fontWeight: 500 }}>Metrics</Typography>}
        >
