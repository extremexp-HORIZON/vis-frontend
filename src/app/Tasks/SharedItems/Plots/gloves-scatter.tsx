...
import { FormControl, InputLabel, Select, MenuItem, Box, Grid, Switch, Typography } from '@mui/material';
import ResponsiveVegaLite from '../../../../shared/components/responsive-vegalite';

interface EffCostAction {
  eff: number;
  cost: number;
}
...
const GlovesScatter = ({ data1, data2, eff_cost_actions }: GlovesScatterProps) => {
  ...
  return (
    <>
      <Box className="panel" style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px', flexWrap: 'wrap',marginTop: '20px' }}>
        <FormControl variant="outlined" style={{ minWidth: 200, marginRight: '20px'  }} disabled={dimensionalityReduction}>
      ...
      <ResponsiveVegaLite
            spec={sharedLegendSpec(transformData(data1), transformData(data2.appliedAffectedActions))}
            actions={false}
            minWidth={1000}          
            minHeight={1000}
            maxHeight={1000}
        ...
        <ResponsiveVegaLite spec={Colorspec(transformData(data1))} actions={false} minWidth={100} minHeight={100} maxHeight={400} maxWidth={400} aspectRatio={1} />
