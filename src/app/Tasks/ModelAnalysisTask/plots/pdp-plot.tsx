...
const PdpPlot = (props: PdpPlotProps) => {
    ...
    const spec={
        ...
        encoding: {
          x: {
            field: tab?.workflowTasks.modelAnalysis?.pdp?.data?.xAxis.axisName || "xAxis default",
            type:
            tab?.workflowTasks.modelAnalysis?.pdp?.data?.xAxis.axisType === "numerical"
                ? "quantitative"
                : "ordinal",
            // aggregate: "mean"
          },
          y: {
            field: tab?.workflowTasks.modelAnalysis?.pdp?.data?.yAxis.axisName || "yAxis default",
            type:
            tab?.workflowTasks.modelAnalysis?.pdp?.data?.xAxis.axisType === "numerical"
                ? "quantitative"
                : "ordinal",
