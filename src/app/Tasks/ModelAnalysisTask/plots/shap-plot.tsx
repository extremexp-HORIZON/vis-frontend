...
import { fetchModelAnalysisShapPlot } from '../../../../store/slices/explainabilitySlice';
...
const ShapPlot = () => {
  ...
  const plotModel = tab?.workflowTasks.modelAnalysis?.shapValues;
  const rawData = tab?.workflowTasks.modelAnalysis?.shapValues?.data as unknown as {
  ...
  const spec = {
    ...
    encoding: {
      ...
      tooltip: [
        ...
        { field: 'Importance', type: 'quantitative', title: 'Shap Value', format: '.4f' }
      ]
    },
  };
  ...
  const error = (
    <InfoMessage
      message="Error fetching SHAP values plot."
      type="info"
  ...
  return (
    <ResponsiveCardVegaLite
      spec={spec}
      actions={false}
      title={plotModel?.data?.plotName || 'SHAP Values'}
      maxHeight={400}
