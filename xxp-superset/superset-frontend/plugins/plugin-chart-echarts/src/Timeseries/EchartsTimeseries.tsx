...
export default function EchartsTimeseries({
  ...
  const columnOptions = datasource.columns.map(item => item.column_name);
  const [selectedColumn, setSelectedColumn] = useState(columnOptions[1]);




  
  const rowOptions = [10, 100, 500, 1000, 10000];

  const metricOptions=['AVG','SUM','MIN','MAX']
  ...
  //   setDataMask({ ownState: { rows } });
  //   setSelectedRows(rows);
  // };
  // const handleMetricSelect = (metric) => {
  //   setDataMask({ ownState: { metric } });
  //   setSelectedMetric(metric);
  // };
  // const handleColumnSelect = (column) => {
  //   setDataMask({ ownState: { column } });
  ...
  console.log('datasourcechart',datasource.columns);
  ...
  return (
    <>
      ...
      {/* <button onClick={handleButtonClick}>
        {show100Rows ? 'Show All Rows' : 'Show 100 Rows'}
      </button> */}
      <Echart
        ref={echartRef}
        refs={refs}
...
{/* <select value={selectedRows} onChange={(e) => handleRowSelect(e.target.value)}>
...
<select value={selectedMetric} onChange={(e) => handleMetricSelect(e.target.value)}>
